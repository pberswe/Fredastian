...
function handleInteract() {
  ...
  if (state.mode === "victoryField") {
    const victorySign = getVictorySignRect();
    if (pointInRect(state.player.x, state.player.y, victorySign)) {
      triggerEndingScreen();
      return;
    }
...
function getVictorySignRect() {
  const boardWidth = 200;
  const boardHeight = 80;
  const x = canvas.width - boardWidth - 90;
  const y = canvas.height / 2 - boardHeight - 40;
  return { x, y, w: boardWidth, h: boardHeight };
}

function drawVictorySign() {
  const rect = getVictorySignRect();

  ctx.fillStyle = "#2e1b0c";
  ctx.fillRect(rect.x + rect.w / 2 - 8, rect.y + rect.h, 16, 110);
  ...
  ctx.textAlign = "center";
  ctx.fillText("U did it!", rect.x + rect.w / 2, rect.y + rect.h / 2 + 8);
}
...
function drawEndingScreen() {
  ...
  ctx.textAlign = "center";
  ctx.fillText("U BEAT THE GAME", canvas.width / 2, canvas.height / 2 - 40);
  ctx.font = "30px 'Trebuchet MS', sans-serif";
  ctx.fillStyle = "#f7f8ff";
  ctx.fillText("Take a bow, hero of Fredastian!", canvas.width / 2, canvas.height / 2 + 12);

  drawGoldPile(220, canvas.height - 160, 160, 110);
  drawGoldPile(canvas.width - 380, canvas.height - 180, 180, 120);
  drawDiamond(canvas.width / 2 - 160, canvas.height / 2 + 80, 60);
  drawDiamond(canvas.width / 2 + 180, canvas.height / 2 + 40, 70);
}

function drawGoldPile(x, y, width, height) {
  const barCount = 4;
  const barHeight = height / barCount;
  for (let i = 0; i < barCount; i++) {
    const barY = y + height - barHeight * (i + 1);
    const inset = i * 12;
    const barWidth = width - inset * 2;
    const gradient = ctx.createLinearGradient(0, barY, 0, barY + barHeight);
    gradient.addColorStop(0, "#ffef9c");
    gradient.addColorStop(1, "#d9a520");
    ctx.fillStyle = gradient;
    ctx.fillRect(x + inset, barY, barWidth, barHeight - 4);
    ctx.strokeStyle = "rgba(0,0,0,0.25)";
    ctx.strokeRect(x + inset, barY, barWidth, barHeight - 4);
  }
}

function drawDiamond(centerX, centerY, size) {
  ctx.fillStyle = "#8af5ff";
  ctx.strokeStyle = "#2c8fb3";
  ctx.lineWidth = 3;
  ctx.beginPath();
...
function drawLeftFieldSign() {
  const boardWidth = 160;
  const boardHeight = 70;
  const boardX = 80;
  const boardY = canvas.height / 2 - boardHeight - 60;

  ctx.fillStyle = "#2e1b0c";
  ctx.fillRect(boardX + boardWidth / 2 - 6, boardY + boardHeight, 12, 90);
  ...
  ctx.font = "20px 'Trebuchet MS', sans-serif";
  ctx.textAlign = "center";
  ctx.fillText("Left Field", boardX + boardWidth / 2, boardY + 26);
  ctx.fillText("Welcome!", boardX + boardWidth / 2, boardY + 50);
}
